{ log, el, els, ev, makeObserveble, nodeObserver, newEl, animation } = window.ago.ksk
...
modulesLoaded = new Promise (resolve) ->
	loading = ->
		if `'cart' in window.ago` and `'agolib' in window.ago` and `'wslib' in window.ago`
			do resolve
...
modulesLoaded.then ->
	cart = window.ago.cart
	{ handleInputTel } = window.ago.agolib
	...
	renderTotal = ->
		...
		if current.density is 3 and count > 499
			price = 110
